...
class ItemsApiClient(CustomRequester):
    ...
    def get_items_list(self, expected_status_code=None):
        response = self.send_request('GET', self._BASE_ENDPOINT, expected_status_code=expected_status_code)
        return response

    def create_item(self, item_data, expected_status_code=None):
    ...
    def update_item(self, item_id, new_data=None, expected_status_code=None):
        response = self.send_request('PUT', f'{self._BASE_ENDPOINT}/{item_id}', json=new_data,
                                     expected_status_code=expected_status_code)
        return response

    def delete_item(self, item_id, expected_status_code=None):
